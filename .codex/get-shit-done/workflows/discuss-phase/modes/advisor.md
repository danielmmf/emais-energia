...
> `/var/www/html/emais/.codex/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "/var/www/html/emais/.codex/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @/var/www/html/emais/.codex/agents/gsd-advisor-researcher.md for your role and instructions.
