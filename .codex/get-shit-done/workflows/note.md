...
- **Global scope**: `/var/www/html/emais/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/var/www/html/emais/.codex/notes/`)
...
2. Glob `/var/www/html/emais/.codex/notes/*.md` (if directory exists) — global notes
...
Global (/var/www/html/emais/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `/var/www/html/emais/.codex/notes/` — works in any directory
